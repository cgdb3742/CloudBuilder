...
GUIOptionsContainer::GUIOptionsContainer(GameContext & gameContext) :
	...
	mMusicVolume(gameContext, sf::Vector2f(0.375, 0.25), true),
	mSoundVolume(gameContext, sf::Vector2f(0.375, 0.75), false),
...
void GUIOptionsContainer::setPositionChilds(sf::Vector2f minCorner, sf::Vector2f maxBox)
{
	...
	mMusicVolume.setBoundingBoxCurrent(sf::Vector2f(xsize, ysize));
	mMusicVolume.setPositionAll(minCorner + sf::Vector2f(margin, margin), maxBox - 2.0f * sf::Vector2f(margin, margin));
	mSoundVolume.setBoundingBoxCurrent(sf::Vector2f(xsize, ysize));
	mSoundVolume.setPositionAll(minCorner + sf::Vector2f(margin, margin), maxBox - 2.0f * sf::Vector2f(margin, margin));
...
void GUIOptionsContainer::updateChildsVector()
{
	...
	mChilds.push_back(mMusicVolume);
	mChilds.push_back(mSoundVolume);
